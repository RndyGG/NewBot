    ...
    client.onAddedToGroup(({ groupMetadata: { id }, contact: { name } }) =>
        client.getGroupMembersId(id)
            .then((ids) => {
                ...
                if (ids.length <= 10) {
                    client.sendText(id, 'Maaf, anggota grup minimum adalah 10 pengguna untuk menggunakan bot ini. Selamat tinggal~').then(() => client.leaveGroup(id))
                } else {
                    client.sendText(id, `Hallo anggota group *${name}*, Terimakasih telah invite bot ini kedalam group ini. masukan */menu* untuk fitur bot.`)
                }
            }))
